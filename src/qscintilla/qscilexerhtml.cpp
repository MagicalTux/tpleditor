...
QColor QsciLexerHTML::defaultColor(int style) const
{
    ...
    case ASPPythonClassName:
        return QColor(0x1c,0x02,0xff);

    case HTMLValue:
        return QColor(0x0f,0xa0,0x0f);
...
const char *QsciLexerHTML::keywords(int set) const
{
    if (set == 1)
        return
            ...
            "alink alt archive article aside axis "
            "background bgcolor border "
            "canvas cellpadding cellspacing char charoff charset checked "
            ...
            "datetime declare defer details dir disabled "
            "enctype embed event "
            "face figcaption figure footer for frame frameborder footer "
            "header headers height hgroup href hreflang hspace http-equiv "
            "id ismap label lang language leftmargin link "
            "longdesc "
            "marginwidth marginheight maxlength media method "
            "multiple "
            "name nav nohref noresize noshade nowrap "
            ...
            "scheme scope section selected shape size span src standby "
            "start style summary "
            "tabindex target text time title topmargin type "
            "usemap "
            "valign value valuetype version video vlink vspace "
            "width "
...
QColor QsciLexerHTML::defaultPaper(int style) const
{
    ...
    case ASPAtStart:
				//TPLSTART
        return QColor(0xff,0xf7,0xe5);
        return QColor(0xe8,0xfb,0xff);
